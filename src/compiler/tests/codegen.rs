...
fn test_import_bytecode_generation() {
    ...
    // Verify the chunk has instructions
    assert!(chunk.instruction_count() > 0);

    // Find LoadModule opcode in the generated bytecode
    let mut found_load_module = false;
    let mut offset = 0;
    while offset < chunk.instruction_count() {
        let op = OpCode::from_u8(chunk.read_u8(offset));
        if matches!(
    ...
    assert!(
        found_load_module,
        "LoadModule opcode should be present in bytecode"
    );
}
...
fn test_import_multiple_bytecode_generation() {
    ...
    // Count LoadModule opcodes (should have 2)
    let mut load_module_count = 0;
    let mut offset = 0;
    while offset < chunk.instruction_count() {
        let op = OpCode::from_u8(chunk.read_u8(offset));
        if matches!(
    ...
    assert!(
        load_module_count >= 2,
        "Should have at least 2 LoadModule opcodes for multiple imports"
    );
}
...
fn test_export_bytecode_generation() {
    ...
    // Verify the chunk has instructions (function should be compiled)
    assert!(chunk.instruction_count() > 0);
}
