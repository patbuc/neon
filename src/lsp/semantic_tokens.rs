...
pub fn token_type_to_semantic_index(token_type: &TokenType) -> Option<u32> {
    match token_type {
        ...
        | TokenType::Question => Some(1), // OPERATOR
...
pub fn generate_semantic_tokens(source: &str) -> Vec<SemanticToken> {
    ...
    loop {
        ...
        let semantic_token = encode_semantic_token(&token, token_type_index, prev_line, prev_start);
...
mod tests {
    ...
    fn test_operators_mapped_correctly() {
        ...
        assert_eq!(
            token_type_to_semantic_index(&TokenType::EqualEqual),
            Some(1)
        );
    ...
    fn test_literals_mapped_correctly() {
        ...
        assert_eq!(
            token_type_to_semantic_index(&TokenType::InterpolatedString),
            Some(2)
        );
