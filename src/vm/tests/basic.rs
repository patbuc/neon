...
#[test]
fn break_in_nested_fn_inside_loop_is_compile_error() {
    let program = r#"
        while (true) {
            fn f() {
                break
...
#[test]
fn nested_fn_in_function_can_call_itself_recursively() {
    let program = r#"
        fn outer() {
            fn factorial(n) {
                if (n <= 1) {
    ...
    let result = vm.interpret(program.to_string());
    assert_eq!(Result::Ok, result);
    assert_eq!("120", vm.get_output());
}

#[test]
fn nested_fn_returned_and_called_after_outer_returns() {
    let program = r#"
        fn outer() {
            fn inner(x) {
                return x + 1
